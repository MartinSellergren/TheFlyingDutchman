/// @file bartenderStockManagement.js
/// General module for stock management

//fetch systemEnvironment from localStorage, sets theme
...
//-------------------- End Beir Region -----------------------------
    ...
    //on load, open generated orders tab
    openTab(1);

    //for keyborard search
    ...
    window.onkeydown = function(event) {
        if (!popupIsOpen) {
            if (event.keycode == 32) event.preventDefault();    //prevent space scroll
            keyboardSearch(event);
        }
        else if (event.keyCode == 27) { //popup is open and keypress is esc: close popup
            if (currentTab == 3) {
...
function keyboardSearch(event) {
    ...
    else {  //edit keyboardSearchString and search with it
    ...
    //(previousFounrRow == null only if no bevergae name starts with first typed letter
...
//from here on the functions are called from the tab-specific modules-------------------------------------

//opens a tab, 1:generated orders, 2:incoming inv, 3: beverage edit
function openTab(newTab) {
    ...
    setTextByLanguage(systemEnvironment.language);    //all text is set (since new created elements)
}
...
function setTabButtonsColors(selectedTab) {
    ...
    for (var i=0; i<tabButtons.length; i++) {
        if (selectedTab == i+1) {
            ...
            tabButtons[i].classList.add("tabButtonSelected");   //adds a css class
        }
        else {
            //tabButtons[i].style.backgroundColor = "inherit";
            //tabButtons[i].style.color = "initial";
            tabButtons[i].className = "tabButtons";             //overwrites all other classes
        }
    }
}


/*this function will create a popupContentDiv, fill it up, and add it to the popupDiv*/
/*data="help" for helpPopup / "setting" for settingPopup/
or selected beverage if get orderDetails/incoming inventory/beverage edit
    if from beverage edit, also clicked table row in data, so: data.beverage and data.clickedTableRow*/
function openPopup(data) {
    ...
    defocus.setAttribute("onclick", "");    //disable outside click-exit

    if (data == "help") {
        popupDiv.style.height = "88.61vh";
        defocus.setAttribute("onclick", "closePopup()");    //enable outside click-exit
        ...
        popupDiv.style.height = "79.11vh";
        fillPopupDiv(popupContentDiv, data);
    }
    else if (currentTab == 2) {
        defocus.setAttribute("onclick", "closePopup()");
        popupDiv.style.height = "63.29vh";
        fillPopupDiv(popupContentDiv, data);
    }
    else if (currentTab == 3) {
        popupDiv.style.padding = "1.58vh";
        popupDiv.style.height = "88.61vh";
        beverageEditTabNS.fillPopupDiv(popupContentDiv, data);
    }

    popupDiv.style.display = "block";   //make it visible
    setTextByLanguage(systemEnvironment.language);    //all text is set (since new created elements)
}
...
//fills popupContentDiv for generated orders tab and incoming inventory tab
//data is selected beverage
...
//fills the main table for all the tabs
...
function createTableRow(rowContentList, useCheckboxes, checkboxesChecked, useNumInput, useId) {
    ...
    if (useNumInput) {
        ...
        numInputField.style.width = "7.12vh";
        numInputField.style.border = "0.63vh solid transparent";
...
//test if any checkbox inside the table is selected
...
//disable the top button (top left of table)
...
//returns true if text is a positive integer
