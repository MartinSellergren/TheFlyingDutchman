...
{      
    ...
    categories(); // Listen for when user change the category
    loadAllDrinks();
    toggleButtons();
    enableAutoLogout();
    
    document.getElementById("displayName").innerHTML = systemEnvironment.username;
...
function restoreEnvironment()
{
    ...
    // Shortcut 0
    if(systemEnvironment.shortCut0 != undefined)
        setShortCut(systemEnvironment.shortCut0, "shortCut0");
    
    // Shortcut 1
    if(systemEnvironment.shortCut1 != undefined)
        setShortCut(systemEnvironment.shortCut1, "shortCut1");

    // Shortcut 2
    if(systemEnvironment.shortCut2 != undefined)
        setShortCut(systemEnvironment.shortCut2, "shortCut2");

    // Shortcut 3
    if(systemEnvironment.shortCut3 != undefined)
        setShortCut(systemEnvironment.shortCut3, "shortCut3");

    // Shortcut 4
    if(systemEnvironment.shortCut4 != undefined)
        setShortCut(systemEnvironment.shortCut4, "shortCut4");
...
function categories() {
    document.getElementById("cat").addEventListener("change", function(event) {
        // console.log("val of cat: ", document.getElementById("cat").value);
...
function loadAllDrinks(searchCondition)
{
    ...
    // Get list of beer from the server
    // prepare connection string
    var request = "http://pub.jamaica-inn.net/fpdb/api.php?username=" + systemEnvironment.username + "&password=" + systemEnvironment.password+ "&action=inventory_get";
    ...
    for(var i = 0; i < data.length; i++)
    {
        ...
        drinkImg.src = "http://icons.iconarchive.com/icons/cedarseed/cocktails/128/Godmother-icon.png";
...
/// This function create an autologout function if user are inactive for a particular time
function enableAutoLogout() {
    ...
    /// This function is called when user do not active for a minute
    function logout() {
        showView("autoLogoutPopup");
        var countTime = 60;
        myVar = setInterval(function(){ myTimer() } , 500);
        var bg = document.getElementById("autoLogoutPopup");
        ...
        function myTimer() {
            ...
            if(r < 1)
                ...
                bg.style.background = "rgba(" + r + "," + g +"," + b + ", 0.9)";
    ...
    /// This function do redirect the page and delete the session information
    function realLogout()
    {
        // remove credential
    ...
    /// This function reset the timer and it is called when the user active the page
    function resetTimer() {
        clearTimeout(t);
        restoreAutoLogoutState();
        t = setTimeout(logout,60000);  // time is in milliseconds
        window.clearInterval(myVar);
    }
    
    /// This function hide the greyout area of the page
    function restoreAutoLogoutState()
    {
...
function setShortCut(sourceObj, shortCutId)
{
    ...
    // Set system variable
    systemEnvironment[shortCutId] = sourceObj;
    
    if(!isNotAvailableForSell(convertObj.itemId))
    {
        // Available for sell
        shortCutItem.setAttribute("onclick","addItemToOrder(" + convertObj.itemId + ",'" + convertObj.itemName + "'," + convertObj.itemPrice + ",1)");
        shortCutItem.setAttribute("class","shortCut");
        shortCutItem.innerHTML = "<img src='http://icons.iconarchive.com/icons/cedarseed/cocktails/128/Godmother-icon.png' title='"+ convertObj.itemPrice +"kr'><p>" + convertObj.itemName + "</p>";
    }
    else
    {
        // Not available for sell
        shortCutItem.removeAttribute("onclick");
        shortCutItem.setAttribute("class","shortCutEmpty");
        shortCutItem.innerHTML = "<img src='http://icons.iconarchive.com/icons/cedarseed/cocktails/128/Godmother-icon.png' title='Out of stock'><p>" + convertObj.itemName + "</p>";
    }
...
function checkOutFunc(tmp)
{
    ...
    if(systemEnvironment.language == "en")
    {
        ...
        var tomSt = "<p></p><h1>Check Out</h1><div id='tableContainer'><table>"; 
        ...
        tomSt += "</table></div ><p><span style='float: left; font-size: 1.25em; margin-left: 10px;' data-locale='total'>Total: </span><span style='float: right; font-size: 1.25em; margin-right: 10px;' id='totalOutput'>" + total.toFixed(2) + " kr</span></p><br><br>";
        
        if(tmp == undefined)
            tomSt += "<p id='itemWarn' style='color: red; visibility: hidden;'>Please select item for checkout!</p><p><span class=\"neonButton\"  style=\"margin-top: 5px; width: 75px; float: left;  background-color:\" onclick=\"closeCheckout()\" />Close</span>";
        else
            tomSt += "<p id='itemWarn' style='color: red; visibility: hidden;'>Please select item for checkout!</p><p><span class=\"neonButton\"  style=\"margin-top: 5px; width: 75px; float: left;  background-color:\" onclick=\"resetCheckout()\" />Reset & Close</span>";
        
        tomSt += "<span class=\"neonButton\" style=\"margin-top: 5px; width: 90px; float: left;  background-color:\" onclick=\"uncheckAll()\"  />Uncheck All</span>";
        tomSt += "<span class=\"neonButton\" style=\"margin-top: 5px; width: 90px; float: left;  background-color:\" onclick=\"checkAll()\"  />Check All</span>";
        tomSt += "<span class=\"neonButton\"  style=\"margin-top: 5px; width: 100px; float: right;\" onclick=\"doPayment()\" />Pay</span></p>";
    }
    else if(systemEnvironment.language == "sv")
    {
        var tomSt = "<p></p><h1>Utcheckning</h1><div id='tableContainer'><table>"; 
        ...
        tomSt += "</table></div ><p><span style='float: left; font-size: 1.25em; margin-left: 10px;' data-locale='total'>Totalt: </span><span style='float: right; font-size: 1.25em; margin-right: 10px;' id='totalOutput'>" + total.toFixed(2) + " kr</span></p><br><br>";
    
        if(tmp == undefined)
            tomSt += "</table></div ><p id='itemWarn' style='color: red; visibility: hidden;'>Please select item for checkout!</p><p><span class=\"neonButton\"  style=\"margin-top: 5px; width: 75px; float: left;  background-color:\" onclick=\"closeCheckout()\" />Stäng</span>";
        else
            tomSt += "</table></div ><p id='itemWarn' style='color: red; visibility: hidden;'>Please select item for checkout!</p><p><span class=\"neonButton\"  style=\"margin-top: 5px; width: 75px; float: left;  background-color:\" onclick=\"resetCheckout()\" />återställ & Close</span>";
        
        tomSt += "<span class=\"neonButton\" style=\"margin-top: 5px; width: 90px; float: left;  background-color:\" onclick=\"uncheckAll()\"  />Avmarkera Alla</span>";
        tomSt += "<span class=\"neonButton\" style=\"margin-top: 5px; width: 90px; float: left;  background-color:\" onclick=\"checkAll()\"  />Markera Alla</span>";
        tomSt += "<span class=\"neonButton\" style=\"margin-top: 5px; width: 100px; float: right;\" onclick=\"doPayment()\" />Betala</span></p>";
    }
